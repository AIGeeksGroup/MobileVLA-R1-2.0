...
# Modified for MobileVLA-R1 2.0: preserve configuration metadata and validate
# projector dimensions while retaining the upstream checkpoint parameter layout.
...
class DownSampleBlock(nn.Module):
    def forward(self, x):
        ...
        if h * w != vit_embeds.shape[1]:
            raise ValueError("Downsample projector requires a square frame token grid")
    ...
    def __init__(self, mm_projector_cfg: MultimodalProjectorConfig, config: PretrainedConfig):
        ...
        if mm_projector_type == "identity":
            if config.mm_hidden_size != config.hidden_size:
                raise ValueError("Identity projector requires matching encoder and LLM dimensions")
            ...
            mlp_gelu_match = re.match(r"^mlp([1-9]\d*)x_gelu$", mm_projector_type or "")
            if mlp_gelu_match:
